...
pad_str = ""
pad = []
guess_hash = lenth_extend_attack(Hash, mlen, append_m)
print("利用长度扩展攻击猜测消息并得到其hash值:",guess_hash)
new_msg = func.bytes_to_list(bytes(m, encoding='utf-8'))
new_msg.extend(pad)
new_msg.extend(func.bytes_to_list(bytes(append_m, encoding='utf-8')))
new_msg_str = m + pad_str + append_m
print("扩展后的消息:",new_msg_str)
new_hash = sm3.sm3_hash(new_msg)
print("对应哈希值:" ,new_hash)
